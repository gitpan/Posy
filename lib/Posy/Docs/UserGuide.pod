...
This describes version B<0.98> of Posy::Docs::UserGuide.
...
A footer part of a page; this is re-generated when a header is re-generated,
but only added to the page when a header-change occurrs (as the footer
part of the section begun with the previous header) or at the end of
the page just before the "foot" part of the page.  This can be used,
for example, if one wishes to place entries inside tables or lists
(and need the table or list to be closed before the next header).
...
True if the page is being generated by the static generation
plugin L<Posy::Plugin::GenStatic> which is used by the L<posy_static>
script to generate static pages.
It is possible for both I<dynamic> and I<static> to be false,
if the CGI script is called from the command-line.
...
Config variables are caught betwixt and between, because some are available
globally, and some are available only in a particular entry, because of the
very flexible way in which you, the user, are able to set up config files
-- see L</Changing Defaults>.  All config variables have a prefix of
'config_'.
...
Whether or not to use entry-index cache files.  (default: true)  Only
turn this off if you have a small site and you find caching annoying.
...
entries.  However, it is also important to note that if you have a lot
of binary files (such as images), it is more efficient to have your
webserver serve them directly, rather than using the Posy::Plugin::BinFile
plugin.
...
since that shouldn't care whether or not the call is dynamic.
...
There are three possible solutions to this.  One is to provide an index file
...
Another possibility depends on why you are maintaining mixed site in the
first place.  If you want to run a simple CGI program as well as Posy, you
can actually stick the CGI program in the data_dir and have Posy treat it
as an entry and run the CGI program for you with the Posy::Plugin::CgiFile
plugin -- but the plugin is limited and will only work on a Linux/Unix
system for simple CGI scripts which don't have setups that are too
complicated.
...
order if you do a reindex_all rather than a reindex.
