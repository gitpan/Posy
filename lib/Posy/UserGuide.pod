...
This describes version B<0.94> of Posy::UserGuide.
...
site.  For larger sites, this can take a while.
...
and then change them on a per-path, per-chunk, per-path-type and/or
per-file basis.
